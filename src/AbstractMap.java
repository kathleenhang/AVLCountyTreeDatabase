...
public abstract class AbstractMap implements Map {
  ...
  protected static class MapEntry implements Entry {
    ...
    private County v;  // value

    public MapEntry(Integer key, County value) {
      k = key;
      v = value;
    }
    ...
    public County getValue() { return v; }

    // utilities not exposed as part of the Entry interface
    protected void setKey(Integer key) { k = key; }
    protected County setValue(County value) {
      County old = v;
      v = value;
      return old;
    }
  ...
  private class KeyIterator implements Iterator<Integer> {
    private Iterator<Entry> entries = entrySet().iterator();   // reuse entrySet
    public boolean hasNext() { return entries.hasNext(); }
    public Integer next() { return entries.next().getKey(); }             // return key!
   ...
   */
  @Override
  public Iterable<Integer> keySet() { return new KeyIterable(); }

  //---------------- nested ValueIterator class ----------------
  private class ValueIterator implements Iterator<County> {
    private Iterator<Entry> entries = entrySet().iterator();   // reuse entrySet
    public boolean hasNext() { return entries.hasNext(); }
    public County next() { return entries.next().getValue(); }           // return value!
    public void remove() { throw new UnsupportedOperationException("remove not supported"); }
  } //----------- end of nested ValueIterator class -----------

  //---------------- nested ValueIterable class ----------------
  private class ValueIterable implements Iterable<County> {
    public Iterator<County> iterator() { return new ValueIterator(); }
   ...
   */
  @Override
  public Iterable<County> values() { return new ValueIterable(); }
}
